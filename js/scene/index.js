...
export default class PlayGame extends Phaser.State {
  ...
  start() {
    ...
    this.placeSquare = function(x, y, value) {
      ...
      var tween = game.add.tween(square.scale).to({
        x: 1.0,
        y: 1.0
      }, 100, Phaser.Easing.Sinusoidal.InOut, true);
    ...
    this.swipeCommon = function(i, j, arrNode, posJson, condition, nextArrNode, nextPosJson) {
      ...
      if (!arrNode.newNode && arrNode.value == this.array[i][j].value) {
        ...
        // 渐渐透明后被kill掉
        var t1 = game.add.tween(arrNode.sprite).to({
          alpha: 0
        ...
        var t2 = game.add.tween(this.array[i][j].sprite).to({
          alpha: 0
        }, duration, Phaser.Easing.Linear.None, true);
      ...
      for (var i = 1; i < this.array.length; i++) {
        for (var j = 0; j < this.array.length; j++) {
          if (this.array[i][j].value != 0) {
            ...
            this.swipeCommon(i, j, this.array[index][j], {
                x: this.transX(index),
                y: this.transY(j)
              },
              index + 1 != i, this.array[index + 1][j], {
                x: this.transX(index + 1),
                y: this.transY(j)
              });
          }
        }
      ...
      for (var i = 0; i < this.array.length; i++) {
        for (var j = 1; j < this.array.length; j++) {
          if (this.array[i][j].value != 0) {
            ...
            this.swipeCommon(i, j, this.array[i][index], {
                x: this.transX(i),
                y: this.transY(index)
              },
              index + 1 != j, this.array[i][index + 1], {
                x: this.transX(i),
                y: this.transY(index + 1)
              });
          }
        }
      }
